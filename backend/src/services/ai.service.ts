...
export class AIService {
  ...
  async speechToTextFPT(audioBuffer: Buffer): Promise<string> {
    ...
    try {
      ...
      if (
        ...
        response.data.hypotheses.length > 0
      ) {
    ...
    try {
      const prompt = `Extract emergency details from this TEXT: "${text}"
            ...
            Return ONLY JSON:
            {
              ...
              "description": "concise summary of important facts only, removing filler words"
            }`;
      ...
      return {
        ...
        searchable_address:
          raw.searchable_address || raw.location_text || "Hanoi, Vietnam",
        incident_type: raw.incident_type || "General",
        people_count:
          typeof raw.people_count === "number" ? raw.people_count : 1,
      ...
      const match = sent.match(
        /(?:tại|ở|địa chỉ|khu vực|số|ngõ)\s+([^,.;!?]+)/i
      );

      if (match) {
        extractedLocation = match[0]
          .replace(/^(tại|ở|địa chỉ|khu vực)\s+/i, "")
          .trim();
  ...
  async analyzeAudio(
    ...
    try {
      ...
      return {
        ...extraction,
        // Return only the refined description (important text), not the raw transcript
        description: extraction.description,
      };
    } catch (error: any) {
      console.warn(
        "⚠️ FPT.AI Failed, falling back to Gemini Multimodal...",
        error.message
      );
      ...
      try {
        const prompt = `
                ...
                Return JSON:
                {
                    ...
                    "people_count": ...,
                    "urgency": "...",
                    "description": "Summary of important details only"
                }
                `;
